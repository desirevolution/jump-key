...
export function handleSearchKeyDown(e, app) {
  ...
  if (e.key === 'ArrowDown') {
    ...
    app.selectedIndex = (app.selectedIndex + 1) % items.length;
  ...
  if (e.key === 'ArrowUp') {
    ...
    app.selectedIndex = (app.selectedIndex - 1 + items.length) % items.length;
...
function buildSearchItems(app) {
  ...
  const { engines, previewEngine, filteredServices } = getSearchState(app);
...
function getSearchState(app) {
  const filteredServices = getFilteredServices(app.categories, app.searchQuery);
  ...
  const command = app.searchQuery.substring(1);
  ...
  if (space === -1) {
    ...
    result.engines = app.searchEngines.filter((engine) =>
      engine.prefix.toLowerCase().startsWith(prefix)
    );
   ...
   * :g hello world
   */
  const prefix = command.substring(0, space).toLowerCase();
...
function scrollSelected(app) {
  setTimeout(() => {
    app.querySelector('.search-item-active')?.scrollIntoView({
      block: 'nearest',
    });
  }, 10);
}
