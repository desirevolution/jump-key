...
export class JkSearchItem extends LitElement {
  ...
  render() {
    // 100% konsistente Styles für JEDES Item in der Liste
    ...
    switch (this.type) {
      case "engine":
        return html`
          ...
          <span class="text-xs ml-2 ${this.active ? "text-indigo-200" : "text-slate-500"}">(${this.data.url})</span>
        `;

      case "preview":
        const terms = this.data.searchTerms?.trim() || "...";
        return html`
          ${this.t("searchEnginePreviewPrefix")} <span class="font-bold text-white">${this.data.name}</span>:
          <span class="italic ${this.active ? "text-indigo-100" : "text-indigo-300"}">"${terms}"</span>
        `;

      case "service":
      default:
        return html`
          <span class="font-medium block sm:inline text-white">${this.data.name}</span>
          <span class="text-xs opacity-60 sm:ml-1">(${this.data.category})</span>
        `;
    }
  }
  ...
  _renderMeta() {
    switch (this.type) {
      case "engine":
        return html`
          <kbd class="px-2 py-0.5 text-xs font-bold rounded shadow ${this.active ? "bg-indigo-700 text-white border-indigo-500" : "bg-slate-900 border border-slate-700 text-indigo-400"}">
            :${this.data.prefix}
          </kbd>
        `;
      ...
      default:
        return this.active
          ? html`<span class="text-xs font-mono bg-indigo-700 px-2 py-0.5 rounded shadow hidden sm:inline">Enter</span>`
          : "";
    }
  }
}
